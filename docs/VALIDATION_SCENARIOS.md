...
## Scenario 6: Composite Adaptation Signal (V2)

**Purpose:** Validates that the three-signal composite adaptation algorithm produces different difficulty decisions than accuracy-only scoring. Documents the V2 novelty claim.

**Signal formula:**
```
composite = 0.60 × accuracy + 0.25 × latency_score + 0.15 × consistency_score

latency_score = clamp((30000 - responseTimeMs) / (30000 - 5000), 0, 1)
consistency_score = clamp(1 - stddev(responseTimes) / 30000, 0, 1)
```

**Case A — Fast correct answers (composite > 0.75 → difficulty increases):**
```
5 answers: all correct, all 5000ms
accuracy=1.0, latency=1.0, consistency=1.0
composite = 0.60×1.0 + 0.25×1.0 + 0.15×1.0 = 1.0 → ADVANCE to Tier 2
```

**Case B — Slow correct answers (latency penalty; composite ~0.77 → still advances):**
```
5 answers: all correct, all 28000ms
accuracy=1.0, latency≈0.08, consistency=1.0
composite ≈ 0.60×1.0 + 0.25×0.08 + 0.15×1.0 = 0.77 → ADVANCE to Tier 2
```

**Case C — 4/5 correct but extremely slow (pure accuracy=0.80 would advance; composite=0.64 → stable):**
```
5 answers: 4 correct, all 29000ms
accuracy=0.80, latency≈0.04, consistency=1.0
composite ≈ 0.60×0.80 + 0.25×0.04 + 0.15×1.0 = 0.48 + 0.01 + 0.15 = 0.64 → MAINTAIN Tier
```

**Thesis significance:**
Case C is the key academic contribution: a patient answering 4/5 correctly but taking 29 seconds per question would have their difficulty INCREASED under V1 (accuracy-only). Under V2, the latency penalty keeps them at the same tier — correctly reflecting the cognitive burden visible in their slow responses. This is the "accuracy without cognitive fluency" scenario that motivates the composite signal.

**Evidence:** `questionEngineComposite.test.ts` — 12 unit tests covering all three signals independently and in combination.

**Sample explanation generated by V2 algorithm:**
> "Accuracy: 80%, Latency: 4%, Consistency: 100% → Composite: 64% within stable band [40%–75%]; maintaining Tier 2."
...
| Composite signal sensitivity | V1 vs V2 distinguishable | Scenario 6C: slow-correct case diverges from accuracy-only result |

### Adaptation Responsiveness Detail

The rolling window is 5 answers (not sessions). The V2 composite algorithm evaluates accuracy + response latency + completion consistency after every answer. A patient answering 5 consecutive questions with high composite score (>0.75) triggers a difficulty increase within a single 10-question session. This is documented in `questionEngineComposite.test.ts`:

```
test: "uses only the last 5 results from a longer array"
→ After 5 consecutive fast+correct answers, composite > 0.75 → difficulty increases immediately
